...
"""
function run_soliton(seq::String, params::DavydovParams=DavydovParams();
                     tmax=20.0, dt=0.01, T=310.0)
    isempty(seq) && throw(ArgumentError("sequence must not be empty"))
    dt > 0 && tmax >= 0 || throw(ArgumentError("dt must be positive and tmax nonnegative"))
    N=length(seq); rng=MersenneTwister(params.seed)
    factors=aa_to_params.(collect(seq)); J=params.J .* first.(factors)
    ...
    saved=0:stride:steps; times=collect(saved).*dt; rho=zeros(N,length(saved))
    ...
    traj=Trajectory(times,rho,us,th,seq,params)
